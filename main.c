...
int count_tokens(const char *input) {
    if (input == NULL || strlen(input) == 0)
        return NULL;
    int numTokens = 0;
    char* localCopy = malloc(strlen(input) + 1);
    strncpy(localCopy, input, strlen(input) + 1);
    char *currentToken;
    do {
        currentToken = strsep(&localCopy, " ");
        if (*currentToken != NULL && *currentToken != '\n')
            ++numTokens;
    } while (localCopy != NULL);
    return numTokens;
}
...
int split_into_strings(const char *input, char ***output) {
    ...
    // Why would we malloc this amount of space? 
    // I must have writting something wrong during the class
    *output = malloc(count_tokens(input)*sizeof(char));
...
void testMakeTokenStrings() {
    // Dette er et array av ord, altså array av array av chars
    char **tokens = 0;
